                        ...
                        <tbody>
                            <c:forEach var="pago" items="${pagos}">
                                <tr>
                                    ...
                                    <td>Q.${pago.getCantidadAPagar()}0</td>
                                    <td>${pago.getIdTipoTarjeta()} </td>
                                    <td>${pago.getFechaPago()}</td>
                                    <td>${pago.getIdUsuario()}</td>
                                    <!--
                                    <td>
                ...
                <div style="margin-top: 1px" class="col bg-white col-sm-4 rounded-end float-end">
                    ...
                    <form action="Controlador?menu=Pago" method="POST">
                        <div class="mb-4">
                            ...
                            <input id="txt1" class="form-control" name="txtCantidadAPagar">
                        </div>
                        <div class="mb-4">
                            <label class="form-label">Tipo Tarjeta</label>
                            <select id="txt2" name="slTipoTarjeta" class="form-select">
                                <c:forEach var="tipoTarjeta" items="${tipoTarjetas}">
                                    <option value="${tipoTarjeta.getIdTipoTarjeta()}">${tipoTarjeta.getTipoTarjeta()}</option>
                                </c:forEach>
                            </select>
                        </div>
                        <div class="mb-4">
                            <label class="form-label">Fecha Pago</label>
                            <input id="txt3" type="date" class="form-control" name="clnFechaPago">
                        </div>
